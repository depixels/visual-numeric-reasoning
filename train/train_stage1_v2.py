...
class VisionEmbeddingModel(nn.Module):
    ...
    def _infer_vision_dim(self, model_name_or_path: str, torch_dtype: torch.dtype) -> int:
        ...
        with torch.no_grad():
            if pv.dim() == 2:
                ...
                try:
                    outputs = self.encoder(pv, grid_thw=grid)
                except TypeError:
                ...
                if pv.dim() == 3:
                    ...
                    pv_b = pv
                try:
                    outputs = self.encoder(pixel_values=pv_b)
                except TypeError:
                    outputs = self.encoder(pv_b)
                x = outputs[0] if isinstance(outputs, tuple) else getattr(outputs, "last_hidden_state", outputs)
    ...
    def __init__(
        ...
        # ✅ 用 dry-run 推断真实视觉输出维度（你的 case 很可能是 2560）
        self.hidden_size = self._infer_vision_dim(model_name_or_path, torch_dtype)
        print(f"[Detect] inferred vision hidden_size = {self.hidden_size}")
    ...
    def _pool_qwen_tokens(self, x: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        for c in counts:
            ...
            if c <= 0:
                raise RuntimeError(f"Non-positive token count computed from grid_thw: {counts}")
            # Global Average Pooling per image
            feats.append(x[offset:offset + c].mean(dim=0))
            offset += c
        ...
        feats = torch.stack(feats, dim=0)
        # CHANGE 2: 这里也不要 normalize，返回 raw features
        return feats
    ...
    def forward(
        ...
        **kwargs
        ...
        # === Step 2: 细粒度适配 (Trainable) ===
        # 将梯度阻断后的 general_feats 喂给 adapter
        # adapter 会学习将“通用的钟表特征”映射到“区分度高的时间特征”
        fine_grained_feats = self.adapter(general_feats)
